// In-memory EVM harness (same approach as the other skills): @ethereumjs/evm + ethers + solc.
// Supports payable calls and native balances so the gated-payment flow can be tested exactly.
...
export const ARTIFACTS = compile([
  { name: 'ReputationGate.sol', path: join(ROOT, 'contracts', 'ReputationGate.sol') },
  { name: 'IReputationScore.sol', path: join(ROOT, 'contracts', 'IReputationScore.sol') },
  { name: 'IValidationRead.sol', path: join(ROOT, 'contracts', 'IValidationRead.sol') },
  { name: 'MockTrust.sol', path: join(ROOT, 'tests', 'fixtures', 'MockTrust.sol') },
]);
