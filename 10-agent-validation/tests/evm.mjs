// In-memory EVM harness (same approach as a2a-mesh / agent-escrow): @ethereumjs/evm
// for execution, ethers Interface for ABI coding, solc compiling on the fly.
...
export const ARTIFACTS = compile([
  { name: 'ValidationRegistry8004.sol', path: join(ROOT, 'contracts', 'ValidationRegistry8004.sol') },
  { name: 'IValidationRegistry8004.sol', path: join(ROOT, 'contracts', 'IValidationRegistry8004.sol') },
  { name: 'IIdentityRegistry8004.sol', path: join(ROOT, 'contracts', 'IIdentityRegistry8004.sol') },
  { name: 'MockIdentity.sol', path: join(ROOT, 'tests', 'fixtures', 'MockIdentity.sol') },
]);
...
export class Chain {
  ...
  async send(abiOrArtifact, to, sender, method, args = []) {
    const abi = Array.isArray(abiOrArtifact) ? abiOrArtifact : abiOrArtifact.abi;
    const iface = new Interface(abi);
    const data = iface.encodeFunctionData(method, args);
    const res = await this.evm.runCall({ caller: addr(sender), to, data: hexToBytes(data), gasLimit: 10_000_000n, ...this.ctx() });
